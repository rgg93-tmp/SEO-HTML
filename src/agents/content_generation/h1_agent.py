...
"""
    ...
    def build_refinement_prompt(
        self, property_data: Dict[str, Any], current_h1: str, suggestion: str, language="English", tone="professional"
    ) -> str:
        return f"""
...
Write in {language} with a {tone} tone. Only output the improved H1 string.
...
"""
    ...
    async def refine(
        ...
        prompt = self.build_refinement_prompt(property_data, current_h1, suggestion, language, tone)
        response = await self.run(task=prompt)
        return response.messages[-1].content.strip()
