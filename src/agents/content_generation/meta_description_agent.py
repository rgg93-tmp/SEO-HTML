...
Generate a meta description for the following property in {language} with a {tone} tone.
Only output the meta description string, under 155 characters. Do not include any extra content, questions, or options.
...
"""
    ...
    def build_refinement_prompt(
        self, property_data: Dict[str, Any], current_meta: str, suggestion: str, language="English", tone="professional"
    ) -> str:
        return f"""
Respond exclusively in {language}. Do not use any other language.
Refine this property meta description based on the suggestion provided. Keep it under 155 characters.
Write in {language} with a {tone} tone. Only output the improved meta description string.
...
Provide only the refined meta description with no explanations or extra content.
"""
    ...
    async def refine(
        ...
        prompt = self.build_refinement_prompt(property_data, current_meta, suggestion, language, tone)
        response = await self.run(task=prompt)
        return response.messages[-1].content.strip()
